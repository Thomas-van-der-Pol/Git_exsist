...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[FINANCE_ACCOUNTANCY_CREDITOR]
...
BEGIN
	...
	SELECT
		CR.ID,
		CR.NUMBER_CREDITOR AS CREDITORNUMBER,
		...
		[ADDRESS_INVOICE].RELATION_ADDRESS_ID AS [VISIT_RELATION_ADDRESS_ID],
	...
	WHERE CR.NUMBER_CREDITOR IS NOT NULL
	  AND CR.ACTIVE = 1
	  AND (
	    CR.EXACT_CREDITOR_LASTSYNC IS NULL
		OR
		...
		OR
		CR.TS_LASTMODIFIED > CR.EXACT_CREDITOR_LASTSYNC
		OR
	  ...
	  AND (
		(((@FILTER IS NULL) OR (@FILTER = '')) OR CR.NUMBER_CREDITOR LIKE '%'+@FILTER+'%')
		OR
