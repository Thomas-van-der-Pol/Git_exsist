...
BEGIN
	...
	SELECT
		...
		CR.ID,
		[ADDRESS].ID,
		COALESCE(CR.FK_CRM_CONTACT_FINANCE, [CONTACT].ID),
		...
		ISNULL(PAP.DESCRIPTION_EXT, AP.DESCRIPTION_EXT) + IIF((FIS.[PERCENTAGE] > 0 AND FIS.[PERCENTAGE] < 100), ' (' + [dbo].[DECIMAL_TO_STRING](FIS.[PERCENTAGE]) + '%)', '')
	...
	JOIN CRM_RELATION CR WITH (NOLOCK) ON CR.ID = COALESCE(P.FK_CRM_RELATION_INVOICE,P.FK_CRM_RELATION_EMPLOYER)
	...
	SELECT
		...
		ISNULL(CR.FK_FINANCE_INVOICE_COLLECT_INTERVAL, [dbo].[FINANCE_INVOICE_COLLECT_INTERVAL_SEPARATED]()),
		1 AS [QUANTITY],
		ROUND((ROUND(ISNULL((FIS.[PERCENTAGE] / 100), 1) * ROUND((PAP.PRICE * ISNULL(PAP.QUANTITY, 1)), 2), 2) * ISNULL((PAP.[COMPENSATION_PERCENTAGE] / 100), 1)), 2) * -1,
		'Vergoeding verzekeraar (' + [dbo].[DECIMAL_TO_STRING](PAP.[COMPENSATION_PERCENTAGE]) + '%)',
	...
	WHERE FIS.ACTIVE = 1
	  ...
	  AND FIS.FK_FINANCE_INVOICE_LINE IS NULL
	  AND FIS.[DATE] <= @DATE
	  AND PAP.COMPENSATED = 1
	  AND PAP.COMPENSATION_PERCENTAGE > 0
