...
class InvoicePrepareController extends AdminBaseController {
    ...
    public function allDatatable(Request $request)
    {
        $this->allColumns = [
            ...
            DB::raw('IIF(FINANCE_BILLCHECK.FK_FINANCE_INVOICE_COLLECT_INTERVAL = [dbo].[FINANCE_INVOICE_COLLECT_INTERVAL_COLLECT_ALL](), 0, ISNULL(FINANCE_BILLCHECK.FK_PROJECT, 0)) AS [FK_PROJECT]'),
            DB::raw('SUM(PRICE_TOTAL) AS PRICE_TOTAL'),
            DB::raw('SUM(PRICE_TOTAL_INCVAT) AS PRICE_TOTAL_INCVAT'),
            DB::raw("STUFF((SELECT ',' + CONVERT(NVARCHAR, FB_DETAIL.ID) FROM FINANCE_BILLCHECK FB_DETAIL WITH (NOLOCK) WHERE FB_DETAIL.FK_CRM_RELATION = FINANCE_BILLCHECK.FK_CRM_RELATION AND FB_DETAIL.FK_FINANCE_INVOICE_COLLECT_INTERVAL = FINANCE_BILLCHECK.FK_FINANCE_INVOICE_COLLECT_INTERVAL AND IIF(FB_DETAIL.FK_FINANCE_INVOICE_COLLECT_INTERVAL = [dbo].[FINANCE_INVOICE_COLLECT_INTERVAL_COLLECT_ALL](), 0, ISNULL(FB_DETAIL.FK_PROJECT, 0)) = IIF(FINANCE_BILLCHECK.FK_FINANCE_INVOICE_COLLECT_INTERVAL = [dbo].[FINANCE_INVOICE_COLLECT_INTERVAL_COLLECT_ALL](), 0, ISNULL(FINANCE_BILLCHECK.FK_PROJECT, 0)) FOR XML PATH, TYPE).value('.[1]','NVARCHAR(MAX)'), 1, 1, '') AS BILLCHECKIDString")
        ];
    ...
    protected function allInternalModifyItems(&$items)
    {
        $items->groupBy([
            ...
            'FINANCE_BILLCHECK.FK_FINANCE_INVOICE_COLLECT_INTERVAL',
            'FINANCE_INVOICE_COLLECT_INTERVAL.DESCRIPTION_SHORT',
            DB::raw('IIF(FINANCE_BILLCHECK.FK_FINANCE_INVOICE_COLLECT_INTERVAL = [dbo].[FINANCE_INVOICE_COLLECT_INTERVAL_COLLECT_ALL](), 0, ISNULL(FINANCE_BILLCHECK.FK_PROJECT, 0))')
        ]);
    }
