...
class DocumentDeletedClient extends BaseMail
{
    ...
    public function build()
    {
        $email = $this->from(config('mail.from.address'), config('mail.from.name'))
            ->subject(KJLocalization::translate('E-mails', 'Document deleted', 'Document deleted', [], $this->language))
            ->embed('logo', public_path('/assets/custom/img/logos/mail/logo.png'))
            ->markdown('consumer.mails.document-deleted-client');
