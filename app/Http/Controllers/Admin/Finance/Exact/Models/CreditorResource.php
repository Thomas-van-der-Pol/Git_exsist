...
class CreditorResource extends ExactBaseResource
{
    ...
    protected function allInternal(Request $request, bool $doDatatableFilter = false, array &$pagination = [], array &$sort = [], $items = null)
    {
        ...
        $items = collect(DB::select('EXEC [FINANCE_ACCOUNTANCY_CREDITOR] ?, ?, ?', [
    ...
    public function export(Request $request)
    {
        ...
        foreach ($items as $item) {
            ...
            try
            {
                $exactId = $relation->EXACT_CREDITOR_ID ? $relation->EXACT_CREDITOR_ID : $accounts->findID("$item->CREDITORNUMBER");
...
//                }
                ...
                // Relatie bijwerken
                $relation->EXACT_CREDITOR_ID = $account->ID;
                $relation->EXACT_CREDITOR_ERROR = '';
                $relation->EXACT_CREDITOR_LASTSYNC = date('Y-m-d H:i:s');
                $relation->save();
            ...
            {
                $relation->EXACT_CREDITOR_ERROR = $e->getMessage();
                $relation->save();
            }
        }
