...
BEGIN
	...
	SELECT
		...
		FI.DATE,
		FI.[PAYMENT_TERM_CODE] AS [PAYMENT_CONDITION],
		FI.PRICE_TOTAL_EXCL,
		FI.PRICE_TOTAL_INCL,
	...
	LEFT JOIN CORE_LABEL CL WITH (NOLOCK) ON CL.ID = CR.FK_CORE_LABEL
	...
	WHERE FI.FK_CORE_WORKFLOWSTATE = [dbo].[CORE_WORKFLOWSTATE_INVOICE_FINAL]()
	  ...
	  AND (FI.EXCLUDE_ACCOUNTANCY IS NULL OR FI.EXCLUDE_ACCOUNTANCY = 0)
	  AND (
